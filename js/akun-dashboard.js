...
async function muatDashboard() {
  ...
  if (sesi.role === "owner") {
    html += `
      <div class="fitur-section">
        <h3>Kelola WiFi Kelas</h3>
        <div class="fitur-card">
          <p class="desc">Upload gambar QR-Code WiFi supaya tampil di halaman WiFi. Hapus untuk kembali ke status "belum tersedia".</p>
          <div id="wifiPreviewArea" class="wifi-preview-area">
            <p class="hint-note">Memuat status QR saat ini...</p>
          </div>
          <div class="jadwal-form" style="margin-top:12px;">
            <label for="inputWifiQr">Gambar QR-Code (upload baru akan menimpa yang lama)</label>
            <input type="file" id="inputWifiQr" accept="image/*" />
            <button class="btn-simpan" id="btnUploadWifiQr">Upload QR</button>
            <button class="btn-logout" id="btnHapusWifiQr" style="margin-top:8px;">Hapus QR</button>
            <p class="hint-note" id="wifiQrStatus"></p>
          </div>
        </div>
      </div>

      <div class="fitur-section">
        <h3>Pemakaian Token AI</h3>
        <div class="fitur-card">
          <p class="desc">Token dihitung dari data resmi yang dikembalikan Groq di setiap balasan chat (bukan tebakan), jadi angkanya sama persis dengan yang tercatat di akun Groq.</p>
          <div id="tokenAiRingkasan" class="token-ai-ringkasan">
            <p class="hint-note">Memuat data pemakaian token...</p>
          </div>
          <div class="tabel-scroll" style="margin-top:14px;">
            <table class="tabel-akun" id="tabelTokenAiHarian">
              <thead>
                <tr><th>Tanggal</th><th>Token Masuk</th><th>Token Keluar</th><th>Total</th><th>Jml Chat</th></tr>
              </thead>
              <tbody id="tbodyTokenAiHarian">
                <tr><td colspan="5">Memuat...</td></tr>
              </tbody>
            </table>
          </div>
...
async function muatKelolaWifiQr() {
  const preview = document.getElementById("wifiPreviewArea");
  const status = document.getElementById("wifiQrStatus");
  const btnUpload = document.getElementById("btnUploadWifiQr");
  const btnHapus = document.getElementById("btnHapusWifiQr");
  const inputFile = document.getElementById("inputWifiQr");
  ...
  async function refreshPreview() {
    try {
      ...
      if (data && data.qr_url) {
        preview.innerHTML = `<img src="${data.qr_url}" alt="QR WiFi saat ini" class="wifi-preview-img" />`;
        btnHapus.style.display = "inline-block";
      } else {
        preview.innerHTML = `<p class="hint-note">Belum ada QR yang diupload. Halaman WiFi menampilkan pesan "belum tersedia".</p>`;
        btnHapus.style.display = "none";
      }
    } catch (err) {
      preview.innerHTML = `<p class="hint-note">Gagal memuat status QR.</p>`;
    }
  }
  ...
  btnUpload.addEventListener("click", async () => {
    ...
    if (!file) {
      status.textContent = "Pilih gambar QR terlebih dahulu.";
      return;
    }
    if (file.size > 3 * 1024 * 1024) {
      status.textContent = "Ukuran gambar maksimal 3 MB.";
      return;
    }
    ...
    try {
      ...
      const namaFile = `wifi-qr-${Date.now()}.${ekstensi}`;
      ...
      const { data: urlData } = supabaseClient.storage.from("wifi-qr").getPublicUrl(namaFile);

      const { error: errUpdate } = await supabaseClient
        .from("wifi_qr")
        .update({
          qr_url: urlData.publicUrl,
          diupload_oleh: sesi.nama,
          diupdate_pada: new Date().toISOString(),
        })
        .eq("id", 1);
      if (errUpdate) throw errUpdate;

      status.textContent = "QR berhasil diupload.";
      inputFile.value = "";
      refreshPreview();
    } catch (err) {
      status.textContent = "Gagal mengunggah QR. Coba lagi.";
    } finally {
  ...
  btnHapus.addEventListener("click", async () => {
    ...
    try {
      const { error } = await supabaseClient
        .from("wifi_qr")
        .update({ qr_url: null, diupload_oleh: null })
        .eq("id", 1);
      if (error) throw error;

      status.textContent = "QR dihapus. Halaman WiFi kembali menampilkan status belum tersedia.";
      refreshPreview();
    } catch (err) {
      status.textContent = "Gagal menghapus QR.";
    } finally {
...
// ---- PEMAKAIAN TOKEN AI (Owner) ----

async function muatTokenAiUsage() {
  const ringkasan = document.getElementById("tokenAiRingkasan");
  const tbody = document.getElementById("tbodyTokenAiHarian");

  try {
    const { data, error } = await supabaseClient
      .from("ai_token_usage")
      .select("tanggal, token_masuk, token_keluar, jumlah_chat")
    ...
    if (!data || data.length === 0) {
      ringkasan.innerHTML = `<p class="hint-note">Belum ada pemakaian token tercatat.</p>`;
      tbody.innerHTML = `<tr><td colspan="5">Belum ada data.</td></tr>`;
      return;
    }

    const totalMasuk = data.reduce((sum, d) => sum + Number(d.token_masuk), 0);
    const totalKeluar = data.reduce((sum, d) => sum + Number(d.token_keluar), 0);
    const totalChat = data.reduce((sum, d) => sum + Number(d.jumlah_chat), 0);
    const totalToken = totalMasuk + totalKeluar;

    ringkasan.innerHTML = `
      <p class="token-total-besar">${formatAngka(totalToken)} <span class="token-total-label">token total</span></p>
      <p class="hint-note">${formatAngka(totalMasuk)} token masuk + ${formatAngka(totalKeluar)} token keluar, dari ${formatAngka(totalChat)} chat (30 hari terakhir)</p>
    `;

    tbody.innerHTML = data
      .map((d) => {
        const total = Number(d.token_masuk) + Number(d.token_keluar);
        const tanggalTampil = new Date(d.tanggal).toLocaleDateString("id-ID", {
          day: "numeric",
          month: "short",
          year: "numeric",
        });
        return `
          <tr>
            <td>${tanggalTampil}</td>
            <td>${formatAngka(d.token_masuk)}</td>
            <td>${formatAngka(d.token_keluar)}</td>
            <td>${formatAngka(total)}</td>
            <td>${formatAngka(d.jumlah_chat)}</td>
          </tr>
        `;
      })
      .join("");
  } catch (err) {
    ringkasan.innerHTML = `<p class="hint-note">Gagal memuat data pemakaian token.</p>`;
